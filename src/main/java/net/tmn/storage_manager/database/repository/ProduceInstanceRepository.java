...
public interface ProduceInstanceRepository extends JpaRepository<ProduceInstance, UUID> {
    ...
    @Query(
            "SELECT pi FROM ProduceInstance pi WHERE pi.status = 'EXPIRED' AND pi.bestBeforeDate <= :date ORDER BY pi.bestBeforeDate")
    List<ProduceInstance> findInstancesExpiredBefore(@Param("date") LocalDate date);

    @Query(
            "SELECT pi FROM ProduceInstance pi WHERE pi.status = 'ACTIVE' AND pi.bestBeforeDate BETWEEN :startDate AND :endDate ORDER BY pi.bestBeforeDate")
    List<ProduceInstance> findActiveInstancesExpiringBetween(
            @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);

    @Query(
            "SELECT pi FROM ProduceInstance pi WHERE pi.produceType.id = :produceTypeId AND pi.status = 'ACTIVE' ORDER BY pi.bestBeforeDate")
